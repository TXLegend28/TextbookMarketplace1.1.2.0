...
class FirebaseRepository @Inject constructor(
    ...
    // --- UPLOAD METHODS (CRITICAL FOR YOUR FIX) ---
    ...
    suspend fun uploadDocument(uri: Uri, bookId: String, fileType: String): Result<String> {
        return try {
            val extension = when (fileType.lowercase()) {
                "pdf" -> "pdf"
                "docx" -> "docx"
                else -> "bin"
            }
            val ref = storage.reference.child("book_documents/$bookId/document.$extension")
            ref.putFile(uri).await()
            val downloadUrl = ref.downloadUrl.await().toString()
            Result.success(downloadUrl)
        } catch (e: Exception) {
